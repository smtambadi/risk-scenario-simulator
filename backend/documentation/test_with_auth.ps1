# Complete Backend E2E Test with Authentication
...
try {
    ...
    Write-Host "JWT Token obtained: $($jwtToken.Substring(0, 50))..." -ForegroundColor Green
} catch {
...
Write-Host "=================================" -ForegroundColor Cyan
Write-Host "ENDPOINT TESTS (WITH JWT TOKEN)" -ForegroundColor Cyan
Write-Host "=================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    $riskCount = if ($risks -is [array]) { $risks.Count } else { 1 }
    Write-Host "✓ Retrieved $riskCount risks" -ForegroundColor Green
    $passCount++
} catch {
...
try {
    ...
    Write-Host "✅ Status: $($response.StatusCode)" -ForegroundColor Green
    $activeRisks = $response.Content | ConvertFrom-Json
    $count = if ($activeRisks -is [array]) { $activeRisks.Count } else { 1 }
    Write-Host "✓ Retrieved $count ACTIVE risks" -ForegroundColor Green
    $passCount++
} catch {
    Write-Host "❌ Error: $($_.Exception.Message)" -ForegroundColor Red
    $failCount++
...
try {
    ...
    Write-Host "✅ Status: $($response.StatusCode)" -ForegroundColor Green
    $securityRisks = $response.Content | ConvertFrom-Json
    $count = if ($securityRisks -is [array]) { $securityRisks.Count } else { 1 }
    Write-Host "✓ Retrieved $count Security risks" -ForegroundColor Green
    $passCount++
} catch {
    Write-Host "❌ Error: $($_.Exception.Message)" -ForegroundColor Red
    $failCount++
...
try {
    ...
    Write-Host "✓ Search found $count matching risks" -ForegroundColor Green
    $passCount++
} catch {
    Write-Host "❌ Error: $($_.Exception.Message)" -ForegroundColor Red
    $failCount++
...
if ($newRiskId) {
    ...
    try {
        ...
        Write-Host "✓ Risk updated - New level: $($updated.riskLevel), Status: $($updated.status)" -ForegroundColor Green
        $passCount++
    } catch {
        Write-Host "❌ Error: $($_.Exception.Message)" -ForegroundColor Red
        $failCount++
